...
/* ═══════════════════════════════════════════════════════════════
   OSINT Platform — AI Intelligence Analyst Panel
...
export default function AiAnalyst({ data }: AiAnalystProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('osint-gemini-key');
    if (saved) {
  ...
  const getHeaders = useCallback((): Record<string, string> => {
    ...
    const savedKey = localStorage.getItem('osint-gemini-key');
    if (savedKey) {
  ...
  const saveApiKey = useCallback(() => {
    ...
    if (key) {
      localStorage.setItem('osint-gemini-key', key);
      setKeySaved(true);
  ...
  const clearApiKey = useCallback(() => {
    localStorage.removeItem('osint-gemini-key');
    setApiKeyInput('');
    setKeySaved(false);
  }, []);
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  <div className="relative">
                    ...
                    <div className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-[var(--alert-green)] animate-osint-pulse" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hud-text text-[11px] text-[var(--text-heading)]">OSINT Platform ANALYST</span>
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-[8px] font-mono text-[var(--text-muted)] leading-relaxed">
                        Your key is stored locally and sent only to the OSINT Platform server. Get a free key at{' '}
                        <a
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        >
                          {msg.role === 'user' ? 'OPERATOR' : 'OSINT Platform ANALYST'}
                        </span>
