...
function OsintMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core' }: OsintMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Create icons — OSINT Platform Unified Palette
      ...
      // ══ OSINT Platform SDK — Lattice Intelligence Mesh ══
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://osint-platform.local';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'OSINT Platform')}</a></div>
          </div>
    ...
    map.on('click', 'scan-targets-dots', (e: any) => {
      ...
      popup(coords, `<div style="${pStyle}border:1px solid rgba(255,61,61,0.5);">
        ...
        <button onclick="window.openOsintIntel({ type: 'ip', ip: '${idSafe(p.id)}' })" style="width:100%;margin-top:8px;padding:6px 12px;background:rgba(255,109,0,0.15);border:1px solid rgba(255,109,0,0.5);color:#FF6D00;font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:bold;letter-spacing:0.1em;border-radius:4px;cursor:pointer;">[ IP INTEL DEEP DIVE ]</button>
      </div>`);
    });
  ...
  // ══ OSINT Platform SDK — Lattice Sensor Mesh ══
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getLayer('osint-3d-buildings')) {
          map.addLayer({
            id: 'osint-3d-buildings',
            source: 'osint-buildings',
      ...
      console.warn('[OSINT Platform] 3D terrain toggle error:', e);
    }
