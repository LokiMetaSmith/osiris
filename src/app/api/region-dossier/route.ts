...
/**
 * OSINT Platform — Region Dossier API
  ...
  try {
    ...
    const geoRes = await fetch(
      `https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json&zoom=5&addressdetails=1`,
      {
        ...
        headers: { 'User-Agent': 'OSINT PlatformIntelPlatform/1.0' },
      }
    );
    ...
    const [countryResult, wikiResult, hosResult] = await Promise.allSettled([
      ...
      (async () => {
        ...
        } catch (e) { console.warn('[OSINT Platform] Country fetch error:', e instanceof Error ? e.message : e); }
        return null;
      })(),
      ...
      (async () => {
        ...
        } catch (e) { console.warn('[OSINT Platform] Wikipedia fetch error:', e instanceof Error ? e.message : e); }
        return null;
      })(),
      ...
      (async () => {
        ...
        } catch (e) { console.warn('[OSINT Platform] Wikidata fetch error:', e instanceof Error ? e.message : e); }
        return null;
      })(),
    ]);
