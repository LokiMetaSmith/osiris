...
/**
 * OSINT Platform — Maritime Intelligence
...
function connectAisStream() {
  ...
  // Map AIS ship types to OSINT Platform categories
  const getOsintShipType = (typeCode: number) => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (parsed.MessageType === "PositionReport" && parsed.Message?.PositionReport) {
        ...
        existing.type = getOsintShipType(staticData.Type);
      }
