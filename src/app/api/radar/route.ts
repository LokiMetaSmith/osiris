...
/**
 * OSINT Platform — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'OSINT Platform/4.2', 'Accept': 'application/json' },
    });

    console.log('[OSINT Platform] IODA response status:', res.status);
    ...
    console.error('[OSINT Platform] IODA fetch error:', error);
