...
/**
 * OSINT Platform — Active Fire & Wildfire Tracking
...
export async function GET() {
  try {
    ...
    for (const url of firmsSources) {
      try {
        const res = await fetch(url, {
          ...
          headers: { 'User-Agent': 'OSINT Platform-Intelligence-Platform/3.5' },
        });
        if (res.ok) {
    ...
    } catch (e) { console.warn('[OSINT Platform] Suppressed EONET error:', e instanceof Error ? e.message : e); }
