# Self-Hosting OSINT Platform with Docker

OSINT Platform ships as a self-contained Next.js standalone build. This guide covers
running it with Docker / Docker Compose, deploying it as a [CasaOS](https://casaos.io)
app, and configuring the optional API keys.

> **TL;DR:** OSINT Platform runs fully **without any API keys**. All core feeds
...
- **`env_file: .env` (`required: false`)** — if a `.env` file exists its
  values are injected into the container; if it's missing, OSINT Platform still starts
  with the keyless feeds.
- **`ports: ${OSINT_PORT:-3000}:3000`** — the web UI. The container always
  listens on 3000; the published **host** port is `OSINT_PORT` (default
  `3000`). Set `OSINT_PORT` in `.env` to remap it, e.g. `OSINT_PORT=3005`
...
Registry, so you can run OSINT Platform without building anything:

```bash
docker pull ghcr.io/aiacos/osint:latest      # or a pinned tag, e.g. :0.1.0
docker run -d --name osint \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  ghcr.io/aiacos/osint:latest
```
...
```bash
docker build -t osint:latest .
docker run -d --name osint -p 3000:3000 --env-file .env --restart unless-stopped osint:latest
```
...
3. OSINT Platform appears on the dashboard with its icon, reachable on host port
   `3000` (or whatever `OSINT_PORT` you set in `.env`).
...
> directly, either build/tag `osint:latest` first
> (`docker build -t osint:latest /path/to/osint`) or set `image:` to a
...
| `SCANNER_KEY` | Shared secret; **must equal the backend's `OSINT Platform_KEY`** | RECON toolkit |

Without `SCANNER_URL`/`SCANNER_KEY` the RECON endpoints return `503` and the
rest of OSINT Platform works normally. Generate a key with `openssl rand -hex 32`.
...
|----------|---------|---------|
| `OSINT_TELEGRAM_CHANNELS` | Comma-separated list of public Telegram channel usernames (no `@`) to scrape for the **Telegram OSINT** map layer. Overrides the curated default set. | `osintdefender,insiderpaper,aljazeeraenglish,nexta_live,war_monitor` |
| `OSINT_PORT` | Host port the compose file publishes (container itself always listens on 3000). | `3000` |
